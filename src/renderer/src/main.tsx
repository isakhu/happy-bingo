...
function App() {
  ...
  function saveBet(value: number) {
    const safe = Math.max(0, Number.isFinite(value) ? value : 0)
    setBetAmount(safe)
    localStorage.setItem('happy-bingo-bet', String(safe))
  }

  function saveCut(value: number) {
    const safe = Math.min(100, Math.max(0, Number.isFinite(value) ? value : 20))
    setCutPercent(safe)
    localStorage.setItem('happy-bingo-cut', String(safe))
  }

  if (!started) return (
    <div className={`app-shell fixed-app ${theme}`}>
      <header className="topbar"><div><div className="brand">HAPPY <span>BINGO</span></div><div className="subtitle">75-BALL BINGO · CARTELLA SELECTION</div></div><div className="header-actions"><div className="status-pill"><span /> READY · OFFLINE</div><button className="icon-button" onClick={() => setSettingsOpen(true)} aria-label="Settings">⚙</button></div></header>
      <main className="selection-screen fixed-content">
        <section className="selection-head"><div><div className="eyebrow">BEFORE THE GAME</div><h1>Select <span>cartella</span> for this game.</h1><p>Choose the printed cartella numbers customers received.</p></div><div className="selection-summary"><strong>{selected.size}</strong><span>SELECTED</span><small>100 CARTELLA TOTAL</small></div></section>
        <section className="cartella-panel selection-card-panel"><div className="panel-heading"><div><h2>ALL 100 CARTELLA</h2><p>{cardSource === 'printed' ? 'Existing printed cards' : 'Generated PDF card set'}</p></div><span>{selected.size} / 100</span></div><div className="cartella-grid">{Array.from({ length: 100 }, (_, i) => i + 1).map(id => <button key={id} className={`cartella ${selected.has(id) ? 'selected' : ''}`} onClick={() => toggleCard(id)}>{String(id).padStart(3, '0')}{selected.has(id) && <b>✓</b>}</button>)}</div></section>
        <div className="selection-bottom"><div className="selection-note">🃏 Physical cartella stay with players · ⚙ PDF & money settings are hidden in Settings</div><button className="start-button" onClick={startGame} disabled={!selected.size}>▶ START GAME</button></div>
        {message && <div className="toast">{message}</div>}
      </main>
      {settingsOpen && <SettingsModal cardSource={cardSource} setCardSource={value => { setCardSource(value); localStorage.setItem('happy-bingo-card-source', value) }} voiceEnabled={voiceEnabled} setVoiceEnabled={value => { setVoiceEnabled(value); localStorage.setItem('happy-bingo-voice', value ? 'on' : 'off') }} theme={theme} setTheme={saveTheme} betAmount={betAmount} setBetAmount={saveBet} cutPercent={cutPercent} setCutPercent={saveCut} generatingPdf={generatingPdf} createPdf={createPdf} onClose={() => setSettingsOpen(false)} />}
    </div>
  )

  return (
    <div className={`app-shell fixed-app ${theme}`}>
      <header className="topbar"><div><div className="brand">HAPPY <span>BINGO</span></div><div className="subtitle">GAME · MANAGER CONTROL</div></div><div className="header-actions"><div className="status-pill live"><span /> GAME LIVE</div><button className="small-control" onClick={() => setPaused(prev => !prev)}>{paused ? '▶ RESUME' : 'Ⅱ PAUSE'}</button><button className="small-control" onClick={callNext}>CALL NUMBER</button><button className="small-control danger" onClick={newGame}>NEW GAME</button><button className="icon-button" onClick={() => setSettingsOpen(true)} aria-label="Settings">⚙</button></div></header>
      <main className="game-screen fixed-content">
        <section className="dashboard-grid">
          <div className="call-panel panel"><div className="eyebrow">NOW CALLING</div><div className="call-ball"><span>{current?.letter || '•'}</span><strong>{current?.number ?? '—'}</strong></div><div className="current-number">{current ? `${current.letter} ${current.number}` : 'READY'}</div><div className="voice-status">🎙 OFFLINE VOICE · {voiceEnabled ? 'ON' : 'OFF'}</div></div>
          <div className="board-panel panel"><div className="panel-heading board-heading"><div><h2>75-NUMBER BOARD</h2><p>Called numbers glow yellow.</p></div><span>{called.length} / 75</span></div><div className="bingo-board">{LETTERS.map((letter, row) => <div className="board-row" key={letter}><div className="board-letter">{letter}</div>{Array.from({ length: 15 }, (_, i) => i + 1 + row * 15).map(n => <span key={n} className={`number-cell ${calledSet.has(n) ? 'called' : ''} ${current?.number === n ? 'latest' : ''}`}>{n}</span>)}</div>)}</div></div>
          <aside className="right-column"><div className="panel history-panel"><div className="panel-heading"><div><h2>CALL HISTORY</h2><p>Last 5 calls</p></div><span>{called.length}</span></div><div className="history-list">{calledHistory.length ? calledHistory.map(item => <div className="history-item" key={`${item.letter}-${item.number}`}><b>{item.letter}</b><strong>{item.number}</strong></div>) : <div className="empty-state">No previous calls</div>}</div><button className="view-button">VIEW ALL</button></div><div className="panel players-panel"><div className="panel-heading"><div><h2>ACTIVE PLAYERS</h2><p>Selected cartella</p></div><span>{selected.size}</span></div><div className="active-list">{Array.from(selected).sort((a, b) => a - b).slice(0, 8).map(id => <span key={id}>👤 {String(id).padStart(3, '0')}</span>)}</div>{selected.size > 8 && <button className="view-button">VIEW ALL</button>}</div><div className="panel verify-panel"><div className="panel-heading"><div><h2>🏆 BINGO CHECK</h2><p>Pause before checking</p></div></div><div className={`pause-badge ${paused ? 'ready' : ''}`}>{paused ? '✓ READY TO CHECK' : '⏸ PAUSE GAME'}</div><div className="verify-row"><input inputMode="numeric" value={verifyInput} onChange={e => setVerifyInput(e.target.value.replace(/\D/g, ''))} placeholder="001–100"/><button onClick={checkWinner}>CHECK</button></div></div></aside>
        </section>
        <section className="live-money-strip"><div><span>PLAYERS</span><strong>{selected.size}</strong></div><div><span>BET</span><strong>{money(betAmount)}</strong></div><div><span>POSSIBLE WIN</span><strong>{money(possibleWin)}</strong></div><div><span>CUT · {cutPercent}%</span><strong>{money(cutAmount)}</strong></div></section>
        ...
        {winner !== null && <div className="winner-overlay"><div><div className="winner-icon">🏆</div><h2>BINGO!</h2><p>Cartella <strong>{String(winner).padStart(3, '0')}</strong> has a valid Bingo.</p><div className="winner-money">Winner receives <strong>{money(possibleWin)}</strong></div><button onClick={() => setWinner(null)}>✓ CONFIRM WINNER</button></div></div>}
        {message && <div className="toast">{message}</div>}
      </main>
      {settingsOpen && <SettingsModal cardSource={cardSource} setCardSource={value => { setCardSource(value); localStorage.setItem('happy-bingo-card-source', value) }} voiceEnabled={voiceEnabled} setVoiceEnabled={value => { setVoiceEnabled(value); localStorage.setItem('happy-bingo-voice', value ? 'on' : 'off') }} theme={theme} setTheme={saveTheme} betAmount={betAmount} setBetAmount={saveBet} cutPercent={cutPercent} setCutPercent={saveCut} generatingPdf={generatingPdf} createPdf={createPdf} onClose={() => setSettingsOpen(false)} />}
    </div>
