...
function App() {
  ...
  const managerCut = Math.floor(totalCollected * Math.min(100, Math.max(0, managerPercent)) / 100)
  const prizePool = Math.max(0, totalCollected - managerCut)
  const remainingWorkingMoney = Math.max(0, workingMoney - managerCut)
  ...
  if (!started) return <div className="app-shell"><header className="topbar"><div><div className="brand">HAPPY <span>BINGO</span></div><div className="subtitle">75-Ball Bingo · One-Screen Game Control</div></div><div className="status-pill"><span /> READY · OFFLINE</div></header><main className="selection-screen"><section className="selection-hero"><div><div className="eyebrow">STEP 1 · SELECT CARTELLA</div><h1>Choose the <span>100 cartella</span> for this game.</h1><p>Click each cartella sold to a player. Selected cartella turn blue. Money settings are visible so an owner can see exactly what the game is doing.</p></div><div className="selection-summary"><strong>{players}</strong><span>SELECTED</span><b>{money(totalCollected)} Br</b><small>Current game money worked</small></div></section><section className="selection-layout"><div className="cartella-panel"><div className="panel-heading"><div><h2>ALL 100 CARTELLA</h2><p>Click to select / click again to remove</p></div><span>{players} / 100</span></div><div className="cartella-grid">{Array.from({ length: 100 }, (_, i) => i + 1).map(id => <button key={id} className={`cartella ${selected.has(id) ? 'selected' : ''}`} onClick={() => toggleCard(id)}>{id}</button>)}</div></div><aside className="selection-side"><div className="side-card money-card"><span className="side-label">MONEY CONTROL</span><label>WORKING MONEY (BIRR)<input type="number" min="0" value={workingMoney} onChange={e => setWorkingMoney(Math.max(0, Math.floor(Number(e.target.value) || 0)))}/></label><label>MANAGER CUT (%)<input type="number" min="0" max="100" value={managerPercent} onChange={e => setManagerPercent(Math.min(100, Math.max(0, Number(e.target.value) || 0)))}/></label><div className="money-line"><span>Default working money</span><b>{money(DEFAULT_WORKING_MONEY)} Br</b></div><div className="money-line"><span>Total money worked</span><b>{money(workedTotal + totalCollected)} Br</b></div><div className="money-line"><span>Current game total</span><b>{money(totalCollected)} Br</b></div><div className="money-line cut-line"><span>Manager cut · {managerPercent}%</span><b>{money(managerCut)} Br</b></div><div className="money-line prize-line"><span>Prize money</span><b>{money(prizePool)} Br</b></div><div className="money-line balance-line"><span>Working money after cut</span><b>{money(remainingWorkingMoney)} Br</b></div></div><div className="side-card"><span className="side-label">GAME SETUP</span><label>ENTRY FEE (BIRR)<input type="number" min="1" value={entryFee} onChange={e => setEntryFee(Math.max(1, Math.floor(Number(e.target.value) || 1)))}/></label><p className="money-note">Financial settings lock when the game starts.</p><button className="start-button" onClick={startGame} disabled={!players}>▶ START GAME <small>{players ? `${players} PLAYER${players === 1 ? '' : 'S'} · ${money(totalCollected)} BR` : 'SELECT CARTELLA FIRST'}</small></button><label className="file-button">ADMIN · REPLACE 100 CARTELLA<input type="file" accept=".csv,text/csv" onChange={e => e.target.files?.[0] && importFile(e.target.files[0])}/></label></div></aside></section>{message && <div className="message">{message}</div>}</main></div>

  return <div className="app-shell"><header className="topbar game-top"><div><div className="brand">HAPPY <span>BINGO</span></div><div className="subtitle">GAME #{String(gameNumber).padStart(4, '0')} · {players} CARTELLA · {players} PLAYERS</div></div><div className="game-top-actions"><div className="status-pill"><span /> GAME LIVE</div><button onClick={() => setPaused(p => !p)}>{paused ? '▶ RESUME' : 'Ⅱ PAUSE'}</button><button onClick={callNext}>CALL NOW</button><button onClick={newGame}>NEW GAME</button></div></header><main className="game-screen"><section className="call-hero"><div className="call-ball"><span>{current?.letter || '•'}</span><strong>{current?.number ?? '—'}</strong></div><div className="call-main"><div className="eyebrow">NOW CALLING</div><div className="current-number">{current ? `${current.letter} ${current.number}` : 'READY'}</div><div className="amharic-call">{current ? `${AMHARIC_LETTERS[current.letter]} ${amharicNumber(current.number)}` : 'ጨዋታው ሊጀምር ነው'}</div><div className="call-status">{paused ? '⏸ PAUSED' : started && remaining.length ? '● AUTO CALLING EVERY 5 SECONDS' : 'GAME FINISHED'}</div></div><div className="game-stats"><div><b>{called.length}</b><span>CALLED</span></div><div><b>{remaining.length}</b><span>REMAINING</span></div><div><b>{money(totalCollected)}</b><span>GAME TOTAL BR</span></div></div></section><section className="finance-strip"><div><span>TOTAL MONEY WORKED</span><strong>{money(workedTotal)}</strong><small>Br · accumulated from completed starts</small></div><div><span>CURRENT GAME</span><strong>{money(totalCollected)}</strong><small>Br · {players} × {money(entryFee)}</small></div><div><span>MANAGER CUT</span><strong>{money(managerCut)}</strong><small>Br · {managerPercent}%</small></div><div><span>PRIZE MONEY</span><strong>{money(prizePool)}</strong><small>Br · after manager cut</small></div><div><span>WORKING BALANCE</span><strong>{money(remainingWorkingMoney)}</strong><small>Br · from {money(workingMoney)} Br</small></div></section><section className="game-layout"><div className="board-panel panel"><div className="panel-heading"><div><h2>75-NUMBER BOARD</h2><p>Called numbers glow blue and white</p></div><span>{called.length} / 75</span></div><div className="bingo-board">{LETTERS.map((letter, col) => <div className="bingo-column" key={letter}><div className="board-letter">{letter}</div>{Array.from({length: 15}, (_, i) => i + 1 + col * 15).map(n => <span key={n} className={`number-cell ${calledSet.has(n) ? 'called' : ''} ${current?.number === n ? 'latest' : ''}`}>{n}</span>)}</div>)}</div></div><aside className="game-side"><div className="panel money-live"><div className="panel-heading"><div><h2>OWNER MONEY VIEW</h2><p>Exact numbers, visible at all times</p></div></div><div className="live-money-main"><span>TOTAL MONEY WORKED</span><strong>{money(workedTotal)}</strong><b>BR</b></div><div className="money-line"><span>Current game</span><b>{money(totalCollected)} Br</b></div><div className="money-line cut-line"><span>Manager · {managerPercent}%</span><b>{money(managerCut)} Br</b></div><div className="money-line prize-line"><span>Prize</span><b>{money(prizePool)} Br</b></div><div className="money-line balance-line"><span>Working balance</span><b>{money(remainingWorkingMoney)} Br</b></div></div><div className="panel called-panel"><div className="panel-heading"><div><h2>CALL HISTORY</h2><p>Newest number first</p></div></div><div className="history">{called.slice(0, 12).map((x, i) => <span key={`${x.number}-${i}`} className={i === 0 ? 'latest-history' : ''}>{x.letter}-{x.number}</span>)}</div></div></aside></section><section className="selected-game-cards"><div className="panel-heading"><div><h2>SELECTED CARTELLA · VERIFY BINGO</h2><p>Only the cartella selected before the game are shown.</p></div><span>{players} ACTIVE</span></div><div className="selected-card-list">{selectedCards.map(card => { const winner = winners.find(w => w.card === card.id); return <div className={`selected-card-wrap ${winner ? 'winner-card' : ''}`} key={card.id}><div className="card-toolbar"><strong>CARTELLA {card.id}</strong><button onClick={() => verify(card.id)}>VERIFY BINGO</button></div><BingoCard card={card} called={calledSet} highlight={winner ? new Set(winner.indexes) : undefined}/></div> })}</div></section>{message && <div className="message">{message}</div>}</main></div>
}
