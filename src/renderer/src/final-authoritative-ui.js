...
(() => {
  ...
  const setCartellaNumber = () => {
    ...
    badge.style.cssText = 'position:absolute;left:16px;top:50%;transform:translateY(-50%);z-index:5;color:#fff;font:1000 14px Arial,Helvetica,sans-serif;letter-spacing:1px;text-shadow:2px 2px 0 #7B5C12;white-space:nowrap;';
  };
  ...
  const ensureSettingsBalance = () => {
    ...
    if (!panel) {
      ...
      if (span) span.style.cssText = 'font-weight:1000;letter-spacing:1px;';
  ...
  const hideGameplayDiagnostics = () => document.querySelectorAll('.bingo-mode .toast').forEach(el => { el.style.display = 'none'; });
