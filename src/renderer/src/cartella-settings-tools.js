(() => {
  ...
  function install() {
    ...
    if (!settings || document.getElementById(TOOL_ID)) return
    ...
    tools.innerHTML = '<h3>CARTELLA TOOLS</h3><p>Clear every Cartella currently selected for the next game. This does not delete the installed 100-Cartella set.</p><button type="button" id="hb-clear-all-cartellas" class="clear-selection-button" style="width:100%;margin-top:8px;">CLEAR ALL SELECTED CARTELLAS</button>'
    ...
    tools.querySelector('#hb-clear-all-cartellas')?.addEventListener('click', () => {
      ...
      if (clear && !clear.disabled) {
        ...
        window.setTimeout(() => { if (button) button.textContent = 'CLEAR ALL SELECTED CARTELLAS' }, 1200)
      } else {
        const message = document.querySelector('.toast')
        if (message) message.textContent = 'NO CARTELLAS ARE CURRENTLY SELECTED.'
      }
    })
  }
