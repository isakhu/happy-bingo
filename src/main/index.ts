...
function createWindow(){const win=new BrowserWindow({width:1400,height:900,minWidth:1100,minHeight:700,title:'Happy Bingo',backgroundColor:'#071a3a',autoHideMenuBar:true,webPreferences:{preload:join(__dirname,'../preload/index.mjs'),contextIsolation:true,nodeIntegration:false,sandbox:false}});win.maximize();load(win)}
...
function authFile(){return join(app.getPath('userData'),'auth.json')}
function legacyAuthFile(){return join(process.env.APPDATA||process.cwd(),'Happy Bingo','auth.json')}
function derive(password:string,salt:Buffer){return scryptSync(password,salt,64)}
async function readAuthRecord():Promise<AuthRecord|null>{const current=authFile();try{const value=JSON.parse(await readFile(current,'utf8'));if(typeof value?.salt==='string'&&typeof value?.hash==='string')return value}catch{}const legacy=legacyAuthFile();if(legacy!==current){try{const value=JSON.parse(await readFile(legacy,'utf8'));if(typeof value?.salt==='string'&&typeof value?.hash==='string'){await mkdir(join(app.getPath('userData')), {recursive:true});await writeFile(current,JSON.stringify(value),'utf8');return value}}catch{}}return null}
async function savePassword(password:string){const salt=randomBytes(32);const hash=derive(password,salt);await mkdir(join(app.getPath('userData')), {recursive:true});await writeFile(authFile(),JSON.stringify({salt:salt.toString('hex'),hash:hash.toString('hex')}),'utf8')}
async function verifyPassword(password:string){const record=await readAuthRecord();if(!record)return false;try{const expected=Buffer.from(record.hash,'hex');const actual=derive(password,Buffer.from(record.salt,'hex'));return expected.length===actual.length&&timingSafeEqual(expected,actual)}catch{return false}}

app.whenReady().then(async()=>{await ensureVoices();ipcMain.handle('auth-status',async()=>({needsSetup:!(await readAuthRecord())}));ipcMain.handle('auth-setup',async(_,currentDefault:string,newPassword:string,confirmPassword:string)=>{if(currentDefault!==DEFAULT_PASSWORD)return{ok:false,error:'The default password is incorrect.'};if(newPassword.length<6)return{ok:false,error:'Password must be at least 6 characters.'};if(newPassword!==confirmPassword)return{ok:false,error:'Passwords do not match.'};await savePassword(newPassword);return{ok:true}});ipcMain.handle('auth-unlock',async(_,password:string)=>({ok:await verifyPassword(password),error:'Incorrect password.'}));ipcMain.handle('play-voice',async(_,file:string)=>getVoiceData(file));ipcMain.handle('voice-health',async()=>{const dir=await ensureVoices();const files:string[]=[];for(const f of ALLOWED_VOICES){try{await readFile(join(dir,f));files.push(f)}catch{}}return{available:files.length,total:ALLOWED_VOICES.size,files}});ipcMain.handle('get-installed-set',async()=>getInstalledSet());protocol.handle(AUDIO_SCHEME,async(req)=>{const name=decodeURIComponent(new URL(req.url).pathname).replace(/^\/+/, '');const canonical=VOICE_BY_KEY.get(name.toLowerCase());if(!canonical)return new Response('Not found',{status:404});const dir=await ensureVoices();try{const data=await readFile(join(dir,canonical));return new Response(data,{status:200,headers:{'Content-Type':'audio/mpeg','Cache-Control':'no-store','Accept-Ranges':'bytes'}})}catch{return new Response('Not found',{status:404})}});createWindow();app.on('activate',()=>{if(BrowserWindow.getAllWindows().length===0)createWindow()})})
