...
async function readAuthRecord():Promise<AuthRecord|null>{const current=authFile();try{const value=JSON.parse(await readFile(current,'utf8'));if(typeof value?.salt==='string'&&typeof value?.hash==='string')return value}catch{}const legacy=legacyAuthFile();if(legacy!==current){try{const value=JSON.parse(await readFile(legacy,'utf8'));if(typeof value?.salt==='string'&&typeof value?.hash==='string'){await mkdir(authDir(),{recursive:true});await writeFile(current,JSON.stringify(value),'utf8');return value}}catch{}}return null}
async function savePassword(password:string){const salt=randomBytes(32);const hash=derive(password,salt);await mkdir(authDir(),{recursive:true});await writeFile(authFile(),JSON.stringify({salt:salt.toString('hex'),hash:hash.toString('hex')}),'utf8');const saved=await verifyPassword(password);if(!saved)throw new Error('Password verification after save failed.')}
...
app.whenReady().then(async()=>{
  ipcMain.handle('auth-status',async()=>({needsSetup:!(await readAuthRecord())}))
  ipcMain.handle('auth-setup',async(_,currentDefault:string,newPassword:string,confirmPassword:string)=>{if(currentDefault!==DEFAULT_PASSWORD)return{ok:false,error:'The default password is incorrect.'};if(newPassword.length<6)return{ok:false,error:'Password must be at least 6 characters.'};if(newPassword!==confirmPassword)return{ok:false,error:'Passwords do not match.'};try{await savePassword(newPassword);return{ok:true}}catch{return{ok:false,error:'Could not save the new password on this computer.'}}})
  ipcMain.handle('auth-unlock',async(_,password:string)=>({ok:await verifyPassword(password),error:'Incorrect password.'}))
