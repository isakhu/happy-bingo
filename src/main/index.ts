...
async function readAuthRecord():Promise<AuthRecord|null>{const current=authFile();try{const value=JSON.parse(await readFile(current,'utf8'));if(typeof value?.salt==='string'&&typeof value?.hash==='string')return value}catch{}const legacy=legacyAuthFile();if(legacy!==current){try{const value=JSON.parse(await readFile(legacy,'utf8'));if(typeof value?.salt==='string'&&typeof value?.hash==='string'){await mkdir(app.getPath('userData'), {recursive:true});await writeFile(current,JSON.stringify(value),'utf8');return value}}catch{}}return null}
async function savePassword(password:string){const salt=randomBytes(32);const hash=derive(password,salt);await mkdir(app.getPath('userData'), {recursive:true});await writeFile(authFile(),JSON.stringify({salt:salt.toString('hex'),hash:hash.toString('hex')}),'utf8')}
