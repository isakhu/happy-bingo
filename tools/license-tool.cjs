...
function genKeys(privatePath, publicPath) {
  ...
  fs.writeFileSync(privatePath, privateKey.export({ type: 'pkcs8', format: 'pem', cipher: 'aes-256-cbc', passphrase: process.env.HAPPY_BINGO_KEY_PASSWORD || undefined }));
...
function genLicense(privatePath, customerId, expiry) {
  const passphrase = process.env.HAPPY_BINGO_KEY_PASSWORD || undefined;
  const privateKey = crypto.createPrivateKey({ key: fs.readFileSync(privatePath), passphrase });
