...
$('#export').onclick=async()=>{if(validCount()!==100){showMessage(`Finish all 100 Cartellas first. ${validCount()} are valid.`);return}try{const r=await window.cardSetup.saveHbc({setId:$('#setId').value.trim()||'HB-001',cards});if(!r.canceled)showMessage(`Exported ${r.setId}.`,true)}catch(e){showMessage(e?.message||'Could not export the set.')}}
$('#install').onclick=async()=>{if(validCount()!==100){showMessage(`Finish all 100 Cartellas first. ${validCount()} are valid.`);return}try{const r=await window.cardSetup.installHbc({setId:$('#setId').value.trim()||'HB-001',cards});showMessage(`Installed ${r.setId} for Happy Bingo.`,true)}catch(e){showMessage(e?.message||'Could not install the set.')}}
$('#package').onclick=async()=>{if(validCount()!==100){showMessage(`Finish all 100 Cartellas first. ${validCount()} are valid.`);return}try{showMessage('Choose the Happy Bingo customer installer. The result will be one customer .exe containing this 100-card set…');const r=await window.cardSetup.buildCustomerPackage({setId:$('#setId').value.trim()||'HB-001',cards});if(!r.canceled)showMessage(`Customer package created: ${r.path}`,true)}catch(e){showMessage(e?.message||'Could not build the customer package.')}}
render()
